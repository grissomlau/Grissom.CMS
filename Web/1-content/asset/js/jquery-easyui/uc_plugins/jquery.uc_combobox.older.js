...
(function ($) {
    ...
    */
    ...
    function nav(target, dir) {
        var opts = $(target).uc_combobox('options');
        var panel = $(target).uc_combobox('panel');
    ...
    */
    function unselect(target, value) {
        var state = $.data(target, 'uc_combobox');
    ...
    */
    function loadData(target, data, remainText) {
        ...
        var selected = $(target).uc_combobox('getValues');
    ...
    */
    ...
    function doEnter(target) {
        var t = $(target);
        var panel = t.uc_combobox('panel');
        var opts = t.uc_combobox('options');
        var data = t.uc_combobox('getData');
        var item = panel.children('div.combobox-item-hover');
        if (!item.length) {
            item = panel.children('div.combobox-item-selected');
        }
        ...
        if (opts.multiple) {
            if (item.hasClass('combobox-item-selected')) {
                t.uc_combobox('unselect', item.attr('value'));
            } else {
                t.uc_combobox('select', item.attr('value'));
            }
        } else {
            t.uc_combobox('select', item.attr('value'));
            t.uc_combobox('hidePanel');
        }
        var vv = [];
        var values = t.uc_combobox('getValues');
        for (var i = 0; i < values.length; i++) {
            if (findDataItem(data, opts.valueField, values[i])) {
                vv.push(values[i]);
            }
        }
        t.uc_combobox('setValues', vv);
    }
    ...
    */
    function create(target) {
        ...
        $(target).combo($.extend({}, opts, {
            onShowPanel: function () {
                ...
                scrollTo(target, $(target).uc_combobox('getValue'));
        ...
        return this.each(function () {
            ...
            if (state) {
                ...
                state = $.data(this, 'uc_combobox', {
                    options: $.extend({}, $.fn.uc_combobox.defaults, $.fn.uc_combobox.parseOptions(this), options)
                });
                create(this);
                loadData(this, $.fn.uc_combobox.parseData(this));
            }
    ...
    $.fn.uc_combobox.methods = {
        options: function (jq) {
            ...
            return $.extend($.data(jq[0], 'uc_combobox').options, {
        ...
        reset: function (jq) {
            return jq.each(function () {
                var opts = $(this).uc_combobox('options');
                if (opts.multiple) {
                    $(this).uc_combobox('setValues', opts.originalValue);
                } else {
                    $(this).uc_combobox('setValue', opts.originalValue);
                }
            });
        },
    ...
    $.fn.uc_combobox.parseOptions = function (target) {
        var t = $(target);
    ...
    $.fn.uc_combobox.parseData = function (target) {
        var data = [];
        var opts = $(target).uc_combobox('options');
    ...
    $.fn.uc_combobox.defaults = $.extend({}, $.fn.combo.defaults, {
        valueField: 'value',
        textField: 'text',
        ...
        onHidePanel: function () {
            ...
            // dhenskr 2013-12-02 if can not found the value in datasource , then clear the selected value

            var values = me.combo('getValues');
            if (opts.multiple) {
                for (var i = 0; i < values.length; i++) {
                    var v = values[i];
                    var item = findDataItem(data, opts.valueField, v);
