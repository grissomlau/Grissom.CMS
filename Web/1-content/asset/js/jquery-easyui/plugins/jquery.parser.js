...
(function ($) {
    $.parser = {
		...
		, 'combobox_m', 'dollar', 'dateRange'],
        parse: function (context) {
            var aa = [];
            for (var i = 0; i < $.parser.plugins.length; i++) {
                var name = $.parser.plugins[i];
                var r = $('.easyui-uc_' + name, context);
                if (r.length) {
                    if (r[name]) {
                        r[name]();
                    } else {
                        ...
                        switch (name) {
                            case 'combobox_m':
                                aa.push({ name: 'combobox' });
                                aa.push({ name: 'uc_' + name, jq: r });
                                break;
                            case 'dollar':
                                aa.push({ name: 'numberspinner' });
                                aa.push({ name: 'uc_numberspinner' });
                                aa.push({ name: 'uc_' + name, jq: r });
                                break;
                            default:
                                aa.push({ name: name });
